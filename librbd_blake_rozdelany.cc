...
// funkce na vypocet blake2 hash: key je to co bude shashovat, hashed je vysledna hash
void hmac(const unsigned char *key, unsigned char *hashed) {

	EVP_MD_CTX *blake;
	blake = EVP_MD_CTX_new();
	unsigned int *md_len;
	EVP_DigestInit_ex(blake, EVP_blake2s256(), NULL);
	EVP_DigestUpdate(blake, key, 32);
	EVP_DigestFinal_ex(blake, hashed, md_len);
	EVP_MD_CTX_free(blake);
...
int aes_init(unsigned char **iv, unsigned char *key_hash, int key_data_len, uint64_t this_sector, EVP_CIPHER_CTX *f) {
	...
	j[1] = 0;
  	...
  	EVP_EncryptInit_ex(f, EVP_aes_256_ecb(), NULL, key_hash, iv1);
...
unsigned char *aes_encrypt(unsigned char *plaintext, int *len, EVP_CIPHER_CTX *e) {
  ...
  // DEBUG
  printf("Delka nezasifrovaneho bloku: %d\n", len);
...
/*
  ...
  unsigned char *ciphertext = (unsigned char *)malloc(c_len * sizeof(char));
  EVP_EncryptUpdate(e, ciphertext, &c_len, plaintext, *len);
  EVP_EncryptFinal_ex(e, ciphertext, &f_len);
  // TEST END
*/
  ...
  EVP_CipherInit_ex(e, NULL, NULL, NULL, NULL, 1);
  EVP_CipherUpdate(e, ciphertext, &f_len, plaintext, *len);
		...
		const char *snap_name)
{
	...
	// tady musis nacist klic ze souboru a pokud existuje, ulozit ho do struktury obrazu
	// pokud klic neexistuje, klic je defaultne prazdny ve strukture
	// pokud klic existuje ale soubor nejde otevrit, tak vrat chybu

	// vynuluj klic ve sturkture v image, vynuluj hash ve stejne strukture
	memset(ictx->evp_key, '\0', 33);
	memset(ictx->evp_hash, '\0', 33);

	char file_key_name[100]; // zatim nevime jak to bude dlouhy
	memset(file_key_name, '\0', 100);

	// pridej slozku
	strcat(file_key_name, "/etc/edss/");

	// preved id poolu na string
	char pool_id[50];
	memset(pool_id, '\0', 50);
	sprintf(pool_id, "%ld", (long)io_ctx.get_id());
	strcat(file_key_name, pool_id); // id poolu

	// vytvor nazev souboru
	...
	if(stat(file_key_name, &file) == 0) {
		...
		if(fp != NULL) {
			...
			// tady vypocitej hash a uloz jako klic do ictx->evp_key

			// klic v souboru je OK, jde nacist 32B + 1 ukoncovaci. Pokud nejde tak:
			if(fgets(ictx->evp_key, 33, fp) == NULL) {
				r = -2;
				fprintf(stderr, "Klic v souboru nema spravnou delku. Soubor: %s\n", file_key_name);
			}
			// pokud jde nacist klic ze souboru
			else {

				// DEBUG
				printf("Klic je ok:\t");
				for(int i = 0; i < 32; i++)
					printf("%x", ictx->evp_key[i]);
				printf("\n");
				// DEBUG END

				// vytvor hash z klice, kteoru ulozis do ictx->evp_hash
				hmac((const unsigned char *)ictx->evp_key, ictx->evp_hash);
...
extern "C" ssize_t rbd_read(rbd_image_t image, uint64_t ofs, size_t len, char *buf) {
	...
	long* fData = (long*)(buf + len);
	bool isCrypted = false;
	for (long* pData = (long*) buf; pData < fData; pData++){
		if (*pData){
			isCrypted = true;
			break;
		}
	...
	if (isCrypted && strlen(ictx->evp_key)) {
			...
			for(uint64_t i = 0; i < len; i += 512) {
			    ...
			    if (aes_init(&iv, (unsigned char *)ictx->evp_hash, 32, this_sector, de)) {	
...
extern "C" ssize_t rbd_write(rbd_image_t image, uint64_t ofs, size_t len, const char *buf) {
	...
	if(strlen(ictx->evp_key)) {
			...
			for(uint64_t i = 0; i < len; i += 512) {
				...
				if (aes_init(&iv, (unsigned char *)ictx->evp_hash, 32, this_sector, f)) {
		    		fprintf(stderr, "Couldn't initialize AES cipher. Key: %s\n", ictx->evp_key);
				}
			...
			memcpy((void *)buf, (const void *)encrypted, len * sizeof(char));
