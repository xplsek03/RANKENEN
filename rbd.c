...
int file_stat(const char* name, struct kstat* stat)
{
    int res;
...
// mike: crypto
static void run_crypto(struct bio *bio, char *evp_key) {
	...
	// je to kurva kernel, radsi to tak necham
	u8 *ptr = ptr2;

    int result = 0;

    // decrypt AES 256 start
	struct crypto_cipher *tfm;
	int i;

	// buffer pro ulozeni provedene krypto operace
	u8 *crypto = kmalloc(4096, GFP_ATOMIC);

	// set it
	tfm = crypto_alloc_cipher("aes", 4, CRYPTO_ALG_ASYNC);
	crypto_cipher_setkey(tfm, evp_key, 32);

    // typ operace: read from image
    // delam nad temito daty decrypt a ulozim je zpet
	if (bio_data_dir(bio) == READ) {

        unsigned nsize = nsect << KERNEL_SECTOR_SHIFT;
        int npages = ((nsize - 1) >> KERNEL_PAGE_SHIFT) + 1;

        // projdi vsechny stranky
        do {
            struct page *page;
            ...
            // dekrypt dat
			memset(crypto, '\0', 4096);
			memcpy(crypto, ptr, this_step);

			// tohle rouovnou prepise buffer
            crypto_cipher_decrypt_one(tfm, ptr, crypto);
    ...
    // typ operace: write to image
    // delam nad temito daty encrypt a ulozim je zpet
	else {

        // projdi bio a dej data do bufferu
        bio_for_each_segment(bvec, bio, iter) {
            ...
            int len = ACCESS_BIO_VEC(bvec).bv_len;
 			...
 			// zakoduj data
			memset(crypto, '\0', 4096);
			memcpy(crypto, src, len);

			// tohle rovnou prepise cilovy buffer
            crypto_cipher_encrypt_one(tfm, ptr, crypto);

            //memcpy(ptr, crypto, len);
            ptr += len;
        }
    ...
    // pokud to vyhodilo chybu pri cteni / zapisu
    if(!result) {
    	...
    	;
  		...
  		//char ch[10];
  		//memset(ch, '\0', 10);
  		//sprintf(ch, "%d", (int)debug_len);
		// otevri soubor s klicem a ty data ktery zpracovavas tak zakoduj
		//struct file *fp;
		// soubor jde otevrit
		//if ((fp = file_open("/root/KRBDTEST", O_WRONLY, 0)) != NULL) {
		//	// klic v souboru je OK, jde nacist 32B + 1 ukoncovaci
		//	file_write(fp, 0, ch, strlen(ch));
		//	file_close(fp);
		//}
  		;
  	}
...
struct rbd_device {
	...
	// mike
	char evp_key[33]; // 32B AES 256 key
	// mike end
...
static int rbd_open(struct block_device *bdev, fmode_t mode)
{
	...
	// vynuluj klic ve strukture
	memset(rbd_dev->evp_key, '\0', 33);

	// uloz klic do sturktury, pokud existuje
	char filename[100]; // nazev souboru
	memset(filename, '\0', 100);

	// cilova slozka na disku
	strcat(filename, "/etc/edss/");
	...
	// preved to na string 
	sprintf(pool_id, "%d", (int)rbd_dev->layout.pool_id);

	strcat(filename, pool_id);
	strcat(filename, "_");

	// image id, mozna pouzit to co bylo v kodu, pii.image_id?
	strcat(filename, rbd_dev->spec->image_id);

	// zjisti jestli existuje soubor s nazvem klice
	struct kstat file;

	if(!file_stat(filename, &file)) {

		// otevri soubor s klicem a ty data ktery zpracovavas tak zakoduj
		struct file *fp;
		
		// soubor jde otevrit
		if ((fp = file_open(filename, O_RDONLY, 0)) != NULL) {

			// klic v souboru je OK, jde nacist 32B + 1 ukoncovaci

			file_read(fp, 0, rbd_dev->evp_key, 32);

			// klic nema 32B, ale soubor existuje
			if (strlen(rbd_dev->evp_key) != 32) {
				
				// DEBUG JAK KLIC VYPADA
				if ((fp = file_open("/root/KRBDTEST", O_WRONLY, 0)) != NULL) {
					file_write(fp, 0, rbd_dev->evp_key, strlen(rbd_dev->evp_key));
					file_close(fp);
				}
				// DEBUG END

			}
			// klic ma spravnou delku a je nacteny v rbd_dev->evp_key
			else {
				;
			}
...
// callback zpracovani jednoho requestu
static void rbd_queue_workfn(struct work_struct *work)
{
	...
	// mike
	if(strlen(rbd_dev->evp_key)) {

		// spust nad BIO v requestu crypto
		run_crypto(rq->bio, rbd_dev->evp_key);
...
static int rbd_init_request(struct blk_mq_tag_set *set, struct request *rq, unsigned int hctx_idx, unsigned int numa_node) {
