		...
		const char *snap_name)
{
	...
	// vynuluj klic ve sturkture v image
...
// radek add
int encrypt(const EVP_CIPHER *evp_cipher, const unsigned char *plaintext, int plaintext_len,
...
// radek add
int decrypt(const EVP_CIPHER *evp_cipher, const unsigned char *ciphertext, int ciphertext_len,
		...
		char *buf)
{
	...
	// pokud je neni disk prazdny a klic existuje v bloku
	if (isCrypted && strlen(ictx->evp_key)){

		printf("RBD READ ENCRYPTED. key: %s\n", ictx->evp_key);

		// radek
		const unsigned char *IV  = (const unsigned char*)"5555555555555555";

		// radek
		// add encryption
		unsigned int sizeOfBuffer = len * sizeof(char);
		const unsigned char *oriData = (const unsigned char *)malloc(sizeOfBuffer);
		memcpy((void *)oriData, (const void *) buf, sizeOfBuffer);
		unsigned char *cryptedStr = (unsigned char *)malloc(sizeOfBuffer);
		decrypt(evp_cipher, oriData, sizeOfBuffer, (const unsigned char *)ictx->evp_key, IV, cryptedStr, NULL);
		memcpy((void *)buf, (const void *)cryptedStr, sizeOfBuffer);
		free((void *) cryptedStr);
		free((void *) oriData);
/*
		printf("\nHere is the message decrypted:\n");
		printf("Length ori %d:\n", (int)len);
		//printf("Length new %d:\n", newLen);
...
{
/*
	printf("\nHere is the message write v0.1 :\n");
	for (unsigned int i = 0; i < len; i++){
		printf("%02X", buf[i]);
		if(i == 50) {
			printf("...\n");
...
*/
	...
	if(strlen(ictx->evp_key)) {
		...
		unsigned char *cryptedStr = (unsigned char *)malloc(sizeOfBuffer);
		encrypt(evp_cipher, oriData, sizeOfBuffer, (const unsigned char *)ictx->evp_key, IV, cryptedStr, NULL);
		memcpy((void *)buf, (const void *) cryptedStr, sizeOfBuffer);
		free((void *) cryptedStr);
		free((void *) oriData);
		// \add encrypt
...
/*
	printf("\nHere is the message write crypted:\n");
	printf("Ori length %d:\n", (int)len);
	//printf("Ori length %d:\n", newLen);
	for (unsigned int i = 0; i < len; i++)
	{
